...
**やるゼ！プラットフォーム** — ｔやるゼ！・ｍやるゼ！ 等のモジュールを統一ID/SSOで束ねるランチャー。
...
```
親: やるゼ！ (このリポジトリ)
├── ｔやるゼ！      (TAmJump/onetouch_app)   施設設備管理
└── ｍやるゼ！         (TAmJump/medadapt)       医療介護連携
```

- やるゼ！プラットフォームは**無料**。Proプランは従来通り各モジュールで個別契約。
...
3. 自動ログイン → ランチャー画面
4. ｔやるゼ！カード「連携する」→ 子ログインID `TEST1-admin` + PW `19800101a` で連携確認
